...
describe('mymoneydapp', () => {
  ...
  // const baseAccount = provider.wallet.publicKey;
  // const baseAccount  = anchor.AnchorProvider.env().wallet.publicKey;
...
async function createMint(provider, authority) {
  ...
  await provider.send(tx, [provider.wallet, mint]);
...
async function createMint_byJson(provider, authority) {
  if (authority === undefined) {
    authority = provider.wallet.publicKey;
  }
  // const mint = anchor.web3.Keypair.generate();
  const Keypair = require('./mint-tokenAccount.json')
  let mint = anchor.web3.Keypair.fromSeed(Uint8Array.from(Keypair.slice(0,32)));
  const instructions = await createMintInstructions(
    provider,
    authority,
    mint.publicKey
...
async function createTokenAccount_byJson(provider, mint, owner){
  const Keypair = require('./to-tokenAccount.json')
  let vault = anchor.web3.Keypair.fromSeed(Uint8Array.from(Keypair.slice(0,32)));
  const tx = new anchor.web3.Transaction();
  tx.add(
    ...(await createTokenAccountInstrs(provider, vault.publicKey, mint, owner))
  );
  const sign = await provider.send(tx, [vault]);
